...
public final class LoggerConverter {
    private LoggerConverter() {}
    public static Logger fromSLF4JLogger(String name, org.slf4j.Logger logger) {
    ...
    private static class SLF4JLogger extends Logger {
         ...
         */
        private SLF4JLogger(String name, org.slf4j.Logger logger) {
            super(name, null);
            this.logger = logger;
        }
        ...
        public void log(LogRecord record) {
            ...
            logUnsupported(record.getLevel(), record.getMessage());
        }
        ...
        private void logUnsupported(Level level, String msg) {
        ...
        private void logUnsupported(Level level, Supplier<String> msgSupplier) {
            if (level == Level.WARNING) {
                logger.warn(msgSupplier.get());
            } else if (level == Level.SEVERE) {
                logger.error(msgSupplier.get());
